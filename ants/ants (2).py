...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 3
    min_range = 0       #the 5th place in front of it
    max_range = 1000       #at most 3 entrance transitions
    ...
    def nearest_bee(self, beehive):
        ...
        index = 0
        while start!=beehive:
            if index >= self.min_range and index<= self.max_range:
                if random_or_none(start.bees):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        list_of_bees1 = list(self.place.bees)
        self.armor = self.armor - amount
        for bee in list_of_bees1:
            Insect.reduce_armor(bee, amount)
        if self.armor <= 0:
            list_of_bees2 = list(self.place.bees)
            for bee in list_of_bees2:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        if self.contained_ant != None or isinstance(other, ContainerAnt):
            return False
        return True
        # END Problem 9
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.ants_doubled = [] #a list for ants that are already doubled
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 13
        if self.Impostor: #if the queenant is fake
            ...
            while location:
                if location.ant:
                    if isinstance(location.ant, ContainerAnt) and location.ant.contained_ant not in self.ants_doubled and location.ant.contained_ant:
                        location.ant.contained_ant.damage *= 2
                        self.ants_doubled.append(location.ant.contained_ant)
                    if location.ant not in self.ants_doubled:
                        location.ant.damage= location.ant.damage*2
                        self.ants_doubled.append(location.ant)
                location = location.exit #updating to next place behind it
        # END Problem 13
...
def make_scare(action, bee):
    ...
    """
    # BEGIN Problem EC
    def scare_it(gamestate):
        if bee.is_reversed == False:
            bee.direction = True
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    regular = bee.action
    other = effect(bee.action, bee)
    def apply_it(gamestate):
        nonlocal duration
        if duration>0:
            other(gamestate)
            duration -= 1
