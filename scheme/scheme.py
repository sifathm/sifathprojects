...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if  scheme_procedurep(eval_first):
            return scheme_apply(eval_first, rest.map(lambda y: scheme_eval(y, env)), env)
        elif isinstance(eval_first, Pair):
            return Pair(scheme_eval(eval_first, env))
        else:
            return SchemeError('SchemeError')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first, lambda_value)
        return target.first
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    while not expressions is nil:
        if expressions.rest is nil and expressions.first!= "#f":
            return scheme_eval(expressions.first, env)
...
def do_or_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    #Previous with while loop and this with recursion. Gotta show off both skills
    if expressions is nil:
        return False
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        each_val = scheme_eval(bindings.first.rest.first,env)
        names = Pair(bindings.first.first, names)
        values = Pair(each_val, values)
